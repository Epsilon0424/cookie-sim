...
def apply_party_buffs(
    ...
    def _apply_party_auto_sets_post_scale_no_scaling():
        ...
        # --- 샬롯(서포터): 영원의 대마술사 세트 ---
        # 속성피해 +30%는 "샬롯과 메인 쿠키 속성이 동일"할 때만 적용
        # buff_amp는 pre-scale에서 이미 반영했으니 여기서는 절대 더하지 않음
        if in_party_char and (main_cookie_name != "샬롯맛 쿠키"):
            ...
            if char_set == "영원의 대마술사 세트":
                ...
                add_elem = float(base.get("all_elem_dmg", 0.0))
                if add_elem:
                    wearer_elem = COOKIE_ELEMENT.get("샬롯맛 쿠키", None)       # 샬롯 속성
                    main_elem   = COOKIE_ELEMENT.get(main_cookie_name, None)    # 메인 속성
    ...
    def _apply_charlotte_party_effects():
        ...
        # 바늘땀/결속 패시브 피해10% (벞증 적용)
        stats["passive_dmg"] = float(stats.get("passive_dmg", 0.0)) + (0.10 * innate_scale)

    # =====================================================
    # 5) 샬롯 아티(희미한 날갯짓) 오라
    #  - 파티(메인)에게 들어오는 것:
    #      (1) 적이 받는 피해 +10%  -> enemy_dmg_taken_inc
    #      (2) 모든 속성 피해 +25% -> buff_all_elem_dmg_raw
    #  - 샬롯 본인 전용(원하면 유지):
    #      패시브 배율/속성 배율 등은 메인=샬롯일 때만
    # =====================================================
    CHAR_WINGS_ENEMY_TAKEN_INC = 0.10   # 받피증 +10%
    CHAR_WINGS_ALL_ELEM_ADD    = 0.25   # 모든속성피해 +25%
    CHAR_WINGS_PASSIVE_MULT    = 1.20   # 샬롯 본인 패시브 배율 x1.20
    ...
    def _apply_charlotte_wings_artifact_aura():
        ...
        # 파티 공통
        stats["enemy_dmg_taken_inc"] = float(stats.get("enemy_dmg_taken_inc", 0.0)) + CHAR_WINGS_ENEMY_TAKEN_INC
        stats["buff_all_elem_dmg_raw"] = float(stats.get("buff_all_elem_dmg_raw", 0.0)) + CHAR_WINGS_ALL_ELEM_ADD
        stats["enemy_passive_taken_inc"] = float(stats.get("enemy_passive_taken_inc", 0.0)) + CHAR_WINGS_ENEMY_TAKEN_INC

        # 샬롯 본인 전용
        if main_cookie_name == "샬롯맛 쿠키":
            stats["passive_dmg_mult"] = float(stats.get("passive_dmg_mult", 1.0)) * CHAR_WINGS_PASSIVE_MULT
...
def summarize_effective_stats(stats: Dict[str, float]) -> Dict[str, Dict[str, float]]:
    ...
    # 공격력(공퍼) 관련
    # -------------------------
    self_atk_pct_add = float(s.get("base_atk_pct", 0.0)) + float(s.get("atk_pct", 0.0))
    party_atk_pct_buff = float(s.get("buff_atk_pct_raw", 0.0))

    if globals().get("ATK_PCT_STACKING_MODE", "ADD") == "MUL":
        equip_atk_mult = (1.0 + self_atk_pct_add) * (1.0 + party_atk_pct_buff)
    else:
        equip_atk_mult = (1.0 + self_atk_pct_add + party_atk_pct_buff)

    buff_atk_mult = float(s.get("buff_atk_mult", 1.0))
    atk_mult = equip_atk_mult * buff_atk_mult

    atk_pct_equiv = atk_mult - 1.0
    atk_pct_sum = atk_pct_equiv if globals().get("ATK_PCT_STACKING_MODE", "ADD") == "MUL" else (self_atk_pct_add + party_atk_pct_buff)
    ...
    return {
        "numeric": {
            ...
            "final_atk_mult_display": float(s.get("final_atk_mult", 0.0)),
            "party_final_atk_display": float(s.get("buff_final_atk_mult", 0.0)),
...
def skill_bonus_mult(stats: Dict[str, float], skill_type: str) -> float:
    if skill_type == "basic":
        return 1.0 + float(stats.get("basic_dmg", 0.0))

    if skill_type == "special":
        return 1.0 + float(stats.get("special_dmg", 0.0))

    if skill_type == "ult":
        return 1.0 + float(stats.get("ult_dmg", 0.0))

    if skill_type == "passive":
        taken  = 1.0 + float(stats.get("enemy_passive_taken_inc", 0.0))
        p_mult = float(stats.get("passive_dmg_mult", 1.0))
        return (1.0 + float(stats.get("passive_dmg", 0.0))) * taken * p_mult
...
def melan_cycle_damage_fast(stats: Dict[str, float], party: List[str]) -> Dict[str, float]:
    ...
    total_damage *= (1.0 + float(stats.get("enemy_dmg_taken_inc", 0.0)))
    dps = total_damage / total_time if total_time > 0 else 0.0
